...
use Dell\Codephp\Controllers\Client\CoursesController as ClientCoursesController;
use Dell\Codephp\Controllers\Client\HomeController;

$routes = new Router();
$routes->get('/',                                       HomeController::class . '@index');
$routes->get('/courses/{id}',                           ClientCoursesController::class . '@show');
$routes->get('/categoriescourses/{idcate}',             ClientCoursesController::class . '@categoriescourses');
...
$routes->mount('/admin', function () use ($routes) {
    ...
    $routes->mount('/courses', function () use ($routes) {
        $routes->match('GET|POST', '/',                 CoursesController::class . '@index');
        $routes->get('/{id}/show',                      CoursesController::class . '@show');
        $routes->get('/test',                           CoursesController::class . '@test');
    ...
    $routes->mount('/categories', function () use ($routes) {
        $routes->match('GET|POST', '/',                 CategoryController::class . '@index');
...
$routes->before('GET|POST', '/admin/*', function () {
    if (!isset($_SESSION['user'])) {
        header('Location: /codephp/auth/login');
        exit();
    }
});
$routes->before('GET|POST', '/admin/.*', function () {
