...
class Content_Courses extends Model
{
    ...
    public function getAllView($id_courses)
    {
        try {
            $sql = 'select * from content_courses where id_courses=:id_courses';
            $stmt = $this->conn->prepare($sql);
    ...
    {
        try {
            $sql = "select * from courses where 1 and idcate ='. $idcate .'";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':idcate', $idcate);
            $stmt->execute();
            return $stmt->fetchAll();
